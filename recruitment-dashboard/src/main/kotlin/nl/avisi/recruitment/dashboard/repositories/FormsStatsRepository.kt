package nl.avisi.recruitment.dashboard.repositories

import nl.avisi.recruitment.dashboard.entities.FormsStats
import org.springframework.data.jpa.repository.JpaRepository
import org.springframework.stereotype.Repository

/**
 * JPA repository for performing CRUD operations on FormsStats entities.
 *
 * This interface provides automatic CRUD functionality for FormsStats entities,
 * including basic operations like create, read, update, and delete. It leverages Spring
 * Data JPA to simplify the interaction with the database layer.
 */
@Repository
interface FormsStatsRepository : JpaRepository<FormsStats, Long>
